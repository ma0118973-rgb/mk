...
const JobCard = ({ job, isMyJob, onDelete }) => {
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 p-4 sm:p-6 hover:border-blue-300 transition-all shadow-sm relative overflow-hidden group cursor-pointer ${expanded ? 'ring-2 ring-blue-500' : ''}`} onClick={() => setExpanded(!expanded)}>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            {isMyJob && (
                <button 
                    ...
                    className="bg-red-100 text-red-600 text-sm px-4 py-2 rounded hover:bg-red-200 transition-colors font-bold flex items-center justify-center w-full sm:w-auto"
                >
                    <i className="fas fa-trash-alt mr-2"></i> Delete
                </button>
            )}
            <button className="bg-slate-900 text-white text-sm px-6 py-2 rounded hover:bg-slate-800 transition-colors shadow-sm font-bold flex items-center justify-center w-full sm:w-auto">
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-4 py-6 sm:py-8 pb-safe sm:pb-32">
      ...
      <div className="bg-gradient-to-r from-blue-900 to-slate-900 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-10 text-center text-white mb-8 sm:mb-12 shadow-2xl relative overflow-hidden">
        <div className="relative z-10 max-w-4xl mx-auto">
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-serif font-bold mb-5 sm:mb-7 leading-tight">Find Your Next Role</h1>
          
          <div className="bg-white p-3 rounded-2xl shadow-xl flex flex-col lg:flex-row gap-3 mb-5">
            <div className="flex-grow flex items-center bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
              <i className="fas fa-search text-gray-400 mr-3"></i>
              <input 
                type="text" 
                placeholder="Job title, keywords..." 
                className="w-full bg-transparent text-gray-900 font-bold focus:outline-none placeholder-gray-400 text-base"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <div className="flex-shrink-0 lg:w-48 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 relative">
              <i className="fas fa-map text-gray-400 absolute left-4 top-1/2 -translate-y-1/2"></i>
              <select 
                className="w-full bg-transparent text-gray-900 font-bold focus:outline-none pl-6 appearance-none cursor-pointer text-base"
             ...
             <div className="flex-shrink-0 lg:w-40 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 relative">
              <select 
                className="w-full bg-transparent text-gray-900 font-bold focus:outline-none appearance-none cursor-pointer text-base"
                value={typeFilter}
          ...
          <button onClick={() => setIsModalOpen(true)} className="inline-flex items-center bg-white/10 backdrop-blur-md border border-white/20 text-white px-6 py-2 rounded-full hover:bg-white/20 transition-all font-bold text-xs sm:text-sm uppercase tracking-widest group">
      ...
      <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-2 border-b border-gray-200 pb-4">
            <h2 className="font-bold text-slate-800 text-lg sm:text-xl">{filteredJobs.length} Openings Found</h2>
